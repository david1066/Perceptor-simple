...
class PerceptorSimple
{
    ...
    function Aprender()
    {
        ...
        $query = $conexion->executeQuery('SELECT x1, x2,x3,x4,y FROM entrenamiento2 ');
        ...
        while ($entre = $conexion->fetchArray($query)) {
            ...
            array_push($and, array($entre['x1'], $entre['x2'],$entre['x3'], $entre['x4'],$entre['y']));
        }
         $and2=array();
       ...
       // return var_dump($and);
        ...
        //calculo y
            ...
            array_push($and2, array($and[$i][$j],$and[$i][$j+1],$and[$i][$j+2],$and[$i][$j+3], $y));
            if($y>=$this->tetha){
